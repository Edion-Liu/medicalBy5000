...
<#assign type="com."+company+"."+project+".orm."+module+".domain."+class>
...
<mapper namespace="com.${vars.company}.${vars.project}.orm.${vars.module}.mapper.${class}Mapper">
	...
	<insert id="save" parameterType="${type}">
		...
		<trim prefix="" suffixOverrides=",">  
			<#list colList as col>
				<if test="@Ognl@isNotEmpty(${col.humpColumnName?uncap_first})">  ${col.columnName}<#if col_has_next>, </#if> </if>
			</#list>
		</trim>
		)
		VALUES
		...
		<trim prefix="" suffixOverrides=",">  
			<#list colList as col>
				<if test="@Ognl@isNotEmpty(${col.humpColumnName?uncap_first})"> <#noparse>#{</#noparse>${col.humpColumnName?uncap_first}}<#if col_has_next>, </#if> </if>
			</#list>
		</trim>
		)
	</insert>
