package com.chuangbu.weixin.shiro.service;
...
import com.chuangbu.weixin.shiro.bean.Permission;
import com.chuangbu.weixin.shiro.bean.Role;
import com.chuangbu.weixin.shiro.bean.ShiroInit;
import com.chuangbu.weixin.shiro.bean.User;
import com.google.common.base.Objects;

@Service
public class UserRealm extends AuthorizingRealm {

	static final Logger logger = LoggerFactory.getLogger(UserRealm.class);
	@Autowired
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		ssUser.setLoginName(token.getUsername());
		List<SsUser> lists = ssUserService.find(ssUser);
		...
		if (doCaptchaValidate(token)) {
			...
			ShiroUser shiroUser = new ShiroUser(ssUser.getId(), null, ssUser.getLoginName(), ssUser.getName(), null);
			// 设置用户session
			Session session = SecurityUtils.getSubject().getSession();
			session.setAttribute("user", ssUser);
			return new SimpleAuthenticationInfo(shiroUser, ssUser.getPassword(), ByteSource.Util.bytes(salt), getName());
		} else {
			return null;
		}
	}
	...
	public static class ShiroUser implements Serializable {
		...
		public ShiroUser(String id, String no, String loginName, String name, List<Role> roles) {
			this.id = id;
			this.no = no;
