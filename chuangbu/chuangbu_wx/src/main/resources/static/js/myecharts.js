var MyECharts = {
	...
	ChartOptionTemplates : {
			...
			var option = {
				...
				legend : {
			        ...
			        formatter: function (name) {
			        	var v_data = pie_datas.data;
			        	for(var i = 0; i < v_data.length; i++){
			        		if(v_data[i].name ==name){
			        			return name + '(' + v_data[i].value+')';
			        		}
			        	}
			        },
		...
		Lines : function(data, name, title,is_stack,isWeek) { //data:数据格式：{name：xxx,group:xxx,value:xxx}...  
